...
public class NewsHeadlineClient implements Client {
  ...
  //public NewsHeadlineClient(NewsHeadlineViewer viewer)
    public NewsHeadlineClient(SubAppContext _appContext, ApplicationContext context) {
        //_viewer = viewer;
    	appContext = _appContext;
    	this.context = context;
    	initializeFids(appContext.getFieldDictionary());
    ...
    protected void processOMMItemEvent(OMMItemEvent event) {
        ...
        if (msg.getDataType() != OMMTypes.NO_DATA) {
            NormalizedEvent nevent = appContext.getNormalizedEvent(event);
	...
	protected void processHeadline(NormalizedEvent nevent) {
        ...
        if (displayName == null || pnac == null || attribtn == null || prodCode == null || storyTime == null || storyDate == null) {
        ...
        if(headline.option() == 1) {
        	System.out.println("stop");
        }
        System.out.println(headline.getText());
        
        NewsStoryClient _storyClient = new NewsStoryClient( appContext, appContext.getServiceName(), headline, context);
