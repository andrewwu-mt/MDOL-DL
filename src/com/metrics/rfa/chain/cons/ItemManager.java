...
public class ItemManager implements Client
{
    ...
    public FieldListDao fieldListDao;
    public Chain chainDbo = new Chain();



    public ItemManager(ChainConsFrame sFrame , ApplicationContext context)
    {
        _sFrame = sFrame;
        _context = context;
    ...
    public void sendRequest(String _itemNames, String _serviceName)
    {
    	if(init){
    		init = false;
    		chainName = _itemNames;
    		chainDbo = chainDao.getByName(chainName);
    	}
    ...
    public void OMMparse(OMMData data)
    {
        ...
        for (Iterator<?> iter = ((OMMIterable)_data).iterator(); iter.hasNext();)
        {
            ...
            if (isLink_Fid(fid) || isLong_Fid(fid))
            {
            	...
            	String chain = eentry.getData().toString();
            	if(chain != null && !"".equals(chain)){
            		Item item = itemDao.getByName(chain);
            		if(item == null){
            			Item itemDbo = new Item();
                		itemDbo.setName(chain);
                		itemDbo.setActive(1);
            			itemDao.save(itemDbo);
						...
						Symbol sym = symbolDao.getByName(chain);
						List<FieldList> list = fieldListDao.getAllList();
						for(int j=0 ; j<list.size() ; j++){
							String field = list.get(j).getName();
							String exp = "["+chain+"]"+field;
							Field fie = new Field();
							fie.setSymbol(sym);
							fie.setName(field);
							fie.setExpression(exp);
							fie.setInsertDate(TimeUtil.getTimeStamp(new Date()));
							fieldDao.save(fie);
						}
            			
            		}
            		
            		Link linkDbo = new Link();
            		linkDbo.setChain(chainDbo);
            		linkDbo.setName(chain);
        			linkDao.save(linkDbo);
